...
def accuracy_contrastive(out1, out2, label):
    euclidean_distance = Function.pairwise_distance(out1, out2)
    #print(euclidean_distance)
    pred = torch.where(euclidean_distance > 2.0, 1., 0.)
    label = torch.squeeze(label)
    print(label)
    print(pred)
    sum = (pred == label).sum()/label.size()[0]
    #print(sum)
    return sum
...
def evaluate(args, model, val_dataloader, criterion, device):
    ...
    with torch.no_grad():
        for train_batch_id, batch in enumerate(val_dataloader):
            ...
            #acc = accuracy_contrastive(out1, out2, label_A)
